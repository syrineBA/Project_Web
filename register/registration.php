...
<php>
    ...
    if (isset($_REQUEST['username'])){
        ...
        $image = stripslashes($_REQUEST['image']); // removes backslashes
        $image = mysqli_real_escape_string($con,$image); //escapes special characters in a string

        $trn_date = date("Y-m-d H:i:s");
        $query = "INSERT into `users` (username, password, email, trn_date,image) VALUES ('$username', '".md5($password)."', '$email', '$trn_date','$image')";
